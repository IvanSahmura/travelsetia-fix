...
public class MenuPesan extends javax.swing.JFrame {
    ...
    private void saveNamesToDatabase() {
        try (Connection conn = Koneksi.bukaKoneksi()) {
            ...
            if (jumlahPenumpang >= 1) {
                saveNameToDatabase(tfNamaPenumpang1.getText(), conn);
            }
            if (jumlahPenumpang >= 2) {
                saveNameToDatabase(tfNamaPenumpang2.getText(), conn);
            }
            if (jumlahPenumpang >= 3) {
                saveNameToDatabase(tfNamaPenumpang3.getText(), conn);
            }
            if (jumlahPenumpang >= 4) {
                saveNameToDatabase(tfNamaPenumpang4.getText(), conn);
            }
        } catch (SQLException e) {
    ...
    private void saveNameToDatabase(String namaPenumpang, Connection conn) throws SQLException {
        String sql = "INSERT INTO detail_transaksi (tanggalTransaksi, jumlahPembayaran, namaPenumpang, idPenerbangan ) VALUES (?, ?, ?, ?)";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
